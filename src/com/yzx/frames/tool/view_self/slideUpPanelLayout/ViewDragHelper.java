...
package com.yzx.frames.tool.view_self.slideUpPanelLayout;
...
/**
 * ViewDragHelper is a utility class for writing custom ViewGroups. It offers a number of useful operations and state tracking for allowing
...
public class ViewDragHelper {
	...
	public static abstract class Callback {
		...
		/**
		 * Called when a child view is captured for dragging or settling. The ID of the pointer currently dragging the captured view is
		 * supplied. If activePointerId is identified as {@link #INVALID_POINTER} the capture is programmatic instead of pointer-initiated.
		 * 
		 * @param capturedChild
		...
		/**
		 * Called when the child view is no longer being actively dragged. The fling velocity is also supplied, if relevant. The velocity
		 ...
		 * {@link #settleCapturedViewAt(int, int)} or {@link #flingCapturedView(int, int, int, int)}. If the Callback invokes one of these
		 * methods, the ViewDragHelper will enter {@link #STATE_SETTLING} and the view capture will not fully end until it comes to a
		 * complete stop. If neither of these methods is invoked before <code>onViewReleased</code> returns, the view will stop in place and
		...
		/**
		 * Called when one of the subscribed edges in the parent view has been touched by the user while no child view is currently
		 * captured.
		 * 
		 * @param edgeFlags
		...
		/**
		 * Called when the given edge may become locked. This can happen if an edge drag was preliminarily rejected before beginning, but
		 * after {@link #onEdgeTouched(int, int)} was called. This method should return true to lock this edge or false to leave it
		...
		/**
		 * Called when the user has started a deliberate drag away from one of the subscribed edges in the parent view while no child view
		...
		/**
		 * Return the magnitude of a draggable child view's horizontal range of motion in pixels. This method should return 0 for views that
		...
		/**
		 * Return the magnitude of a draggable child view's vertical range of motion in pixels. This method should return 0 for views that
		 ...
		 * 
		 * <p>
		 * ViewDragHelper may call this method multiple times for the same view even if the view is already captured; this indicates that a
		 * new pointer is trying to take control of the view.
		 * </p>
		 * 
		 * <p>
		...
		/**
		 * Restrict the motion of the dragged child view along the horizontal axis. The default implementation does not allow horizontal
		 * motion; the extending class must override this method and provide the desired clamping.
		 * 
		 * 
		 * @param child
	 ...
	 *            Parent view to monitor
	 * @param sensitivity
	 *            Multiplier for how sensitive the helper should be about detecting the start of a drag. Larger values are more sensitive.
	 *            1.0f is normal.
	 * @param cb
	 *            Callback to provide information and receive events
	 * @return a new ViewDragHelper instance
	...
	/**
	 * Retrieve the current drag state of this helper. This will return one of {@link #STATE_IDLE}, {@link #STATE_DRAGGING} or
	 * {@link #STATE_SETTLING}.
	 * 
	...
	/**
	 * Enable edge tracking for the selected edges of the parent view. The callback's {@link Callback#onEdgeTouched(int, int)} and
	 * {@link Callback#onEdgeDragStarted(int, int)} methods will only be invoked for edges for which edge tracking has been enabled.
	 * 
	 * @param edgeFlags
	 ...
	 * {@link #continueSettling(boolean)} on each subsequent frame to continue the motion until it returns false. If this method returns
	 * false there is no further work to do to complete the movement.
	 * 
	 * <p>
	 * This operation does not count as a capture event, though {@link #getCapturedView()} will still report the sliding view while the
	...
	/**
	 * Settle the captured view at the given (left, top) position. The appropriate velocity from prior motion will be taken into account. If
	 * this method returns true, the caller should invoke {@link #continueSettling(boolean)} on each subsequent frame to continue the motion
	 * until it returns false. If this method returns false there is no further work to do to complete the movement.
	 * 
	 * @param finalLeft
	...
	/**
	 * Settle the captured view based on standard free-moving fling behavior. The caller should invoke {@link #continueSettling(boolean)} on
	 * each subsequent frame to continue the motion until it returns false.
	 * 
	 * @param minLeft
	 ...
	 *            {@link android.view.View#computeScroll()} or similar methods invoked as part of layout or drawing.
	 ...
	 * (mReleaseInProgress) is the only time it is valid to call {@link #settleCapturedViewAt(int, int)} or
	 ...
	 * 
	 * <p>
	 * The state used to report this information is populated by the methods {@link #shouldInterceptTouchEvent(android.view.MotionEvent)} or
	 * {@link #processTouchEvent(android.view.MotionEvent)}. If one of these methods has not been called for all relevant MotionEvents to
	 * track, the information reported by this method may be stale or incorrect.
	 * </p>
	 * 
	 * @param pointerId
	 ...
	 * {@link #processTouchEvent(android.view.MotionEvent)}. You should only rely on the results of this method after all currently
	 * available touch data has been provided to one of these two methods.
	 * </p>
	 * 
	 * @param directions
	 *            Combination of direction flags, see {@link #DIRECTION_HORIZONTAL}, {@link #DIRECTION_VERTICAL}, {@link #DIRECTION_ALL}
	 ...
	 * 
	 * @param edges
	 *            Edges to check for an initial edge touch. See {@link #EDGE_LEFT}, {@link #EDGE_TOP}, {@link #EDGE_RIGHT},
